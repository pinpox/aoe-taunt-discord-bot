...
func messageCreate(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if matched, _ := regexp.MatchString(`^[0-9]+$`, m.Content); matched {

		tauntNum, err := strconv.ParseInt(m.Content, 10, 64)

		if err != nil {
			fmt.Println("Cound not parse taunt number")
			return
		}

		if tauntNum < 1 || tauntNum > 105 {
			fmt.Println("Invalid taunt number (not from 1 to 105)")
			return
		}

		err = loadSound(fmt.Sprintf("taunts/%v.dca", tauntNum))
		if err != nil {
			fmt.Println("Error loading sound: ", err)
			return
		}
